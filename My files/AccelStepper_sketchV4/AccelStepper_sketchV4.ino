...
float estConstant = 13.66;  // Used to calculate estimated time, Seconds per rotation. (17.42) If you change the motor speed this value needs to be reset. 
...
bool unwind = false;        // flag the causes the winding motor and wire motor to turn until button state is changed
...
void setup() {
  ...
  btVarnish.attachPop(btVarnishPopCallback, &btVarnish);  // Run Wind Motor to Varnish 
  btUnwind.attachPop(btUnwindPopCallback, &btUnwind);  // Run Wind and Wire Motor to Unwind Coil
