...
#define debugln(...)
#endif
//SoftwareSerial HMISerial(A0, A5); // RX, TX
// Define stepper motor connections and motor interface type. Motor interface type must be set to 1 when using a driver:
#define dirPin 5    // Winder motor
#define stepPin 2   // Winder motor
#define dirPin2 6   // Wire motor
#define stepPin2 3  // Wire motor
...
bool Unwind = false;       // flag the causes both motors and to turn in reverse until button state is changed
...
NexDSButton btUnwind = NexDSButton(1, 9, "btUnwind");      // varnish button -- dual state
...
NexText tnTotalTurns = NexText(3, 15, "tnTotalTurns");  // total turns
NexText tnEstTime = NexText(3, 14, "tnEstTime");        // estimated time in whole hours:minutes.
...
void bHomePopCallback(void *ptr) {
  ...
  home = true;
  stop = false;
  digitalWrite(sleepPin, LOW);  //turning on the motors
}
...
// Unwind button
void btUnwindPopCallback(void *ptr) {
  debugln("bUnwind");

  uint32_t dual_state = 0;
  // Need to get it twice.  Why I do not know. Bug or timing issue.  I am not sure.
  btUnwind.getValue(&dual_state);
  btUnwind.getValue(&dual_state);
  debugln(dual_state, DEC);
  if (dual_state == 1) {
    Unwind = true;
...
void setup() {
  ...
  btVarnish.attachPop(btVarnishPopCallback, &btVarnish);           // Run Winder for varnish
  btUnwind.attachPop(btUnwindPopCallback, &btUnwind);              // Unwind Winder and reverse wire  motor
...
void loop() {
  ...
  // Unwind function turns stepper motor until the dual state button is set varnish to false.
